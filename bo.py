...
# =============================================================================
# ORIGINAL PACKAGE STRUCTURE – BEGIN (import-guard tweaks only)
# =============================================================================
"""
...
# ---------------------------------------------------------------------
# (the remainder of the original file stays IDENTICAL and UNCHANGED)
# ---------------------------------------------------------------------
...
class F22Aircraft(_FallbackAircraft):
    """
    Advanced F-22 Raptor implementation with 3-D thrust-vectoring.
    """
...
class F35Aircraft(_FallbackAircraft):
    """
    Advanced F-35 Lightning II implementation with STOVL placeholders.
    """
    def update(self, dt: float = 0.05) -> None:
        if hasattr(self.state,"velocity") and hasattr(self.state,"position"):
            thrust_lift=25.0
...
# ---------------------------------------------------------------------
#  Updated **WarGameEnvironment** – terrain-aware & RF-ready
# ---------------------------------------------------------------------
from pl15_j20_sim.environment.terrain import Terrain
from pl15_j20_sim.environment.weather import Weather
from pl15_j20_sim.environment.rf_environment import RFEnvironment
...
class WarGameEnvironment:
    """
    Lightweight environment for the Taiwan conflict demo.
    """
    def __init__(
        self,
...
def main():
    ...
    # Two aircraft by default: 1 J-20, 1 F-22
...
# ------------------------------------------------------------------
# NEW: 100 J-20 vs. 100 F-35 around Taiwan
# ------------------------------------------------------------------
def run_taiwan_conflict_100v100(dt: float = 1.0,
                                ...
                                frontend_dir: str = "frontend") -> None:
    """
    Launch a scenario with 100 J-20s on the western side of Taiwan
    (representing PLAA bases in the PRC), versus 100 F-35s approaching
    from carrier groups on the eastern side of Taiwan.

    Uses the CRT-based manager (no actual missiles) to simulate
    real-time attrition, streamed live via SSE & optionally displayed
    via a React+D3 front-end.
    """
    ...
    # We'll approximate "Taiwan" as the center around (0, 0),
    # placing J-20 at negative X (west) and F-35 at positive X (east).
    # Each group at altitude ~10k m. We'll arrange them in a grid.
    j20_list = []
    f35_list = []

    # 10x10 formation for each side = 100 planes each
    # Spread them ~200 km from the center, altitude 10000 m.
    # Slight random to avoid perfect overlap.
    spacing = 10.0
    half_grid = 10
    west_x = -200.0  # West side
    east_x =  200.0  # East side
    alt = 10000.0
    ...
    # Start the integrated SSE + React server
...
if __name__ == "__main__":
    # For convenience:
    #   RUN_LIVE_VIS=1       => run the 1v1 J20 vs F22 SSE scenario
    #   RUN_100v100=1        => run the 100 J-20 vs 100 F-35 scenario
    #   otherwise            => run the integrated modern React UI (1 J20, 1 F22)
